...
class DBManager {
	...
	*/
	...
	public function __construct() {
		...
		foreach ($lines as $line) {
			...
			$this->selected = mysqli_select_db($this->client,$dbName) or die(mysqli_error());
	...
	*/
	...
	function insertData($name, $email, $description) {
		...
		try{
			$sql = "CREATE TABLE user(name VARCHAR(255), email VARCHAR(355), description TEXT, mail VARCHAR(100))";
			mysqli_query($this->client, $sql);
		}catch(Exception $e){
				print_r("Table Already Created");
		}
		
		mysqli_query($this->client, "INSERT INTO user(name,email,description,mail) VALUES('$name','$email','$description', 'pending')");
		mysqli_close($con);
    }
